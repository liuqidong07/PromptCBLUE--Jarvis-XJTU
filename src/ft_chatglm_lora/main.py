...
from src.ft_chatglm_lora.peft import PeftModel, LoraConfig, TaskType, get_peft_model
...
def main(parser):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_args.model_name_or_path,
        trust_remote_code=True,
        #add_special_tokens=True, ### For ChatGLMv2
    )
    if tokenizer.eos_token is None: # For ChatGLMv2
    ...
    def preprocess_function_train(examples):
        ...
        for i in range(len(examples[prompt_column])):
            if examples[prompt_column][i] and examples[response_column][i]:
                ...
                if model_args.model_name_or_path != "resources/chatglmv2":
    ...
    def preprocess_function_train_new(examples):
        max_seq_length = data_args.max_source_length + data_args.max_target_length
        ...
        for i in range(len(examples[prompt_column])):
            if examples[prompt_column][i] and examples[response_column][i]:
                ...
                input_ids = tokenizer.build_inputs_with_special_tokens(a_ids, b_ids)    # add bos and eos tokens
                ...
                model_inputs["input_ids"].append(input_ids)
                model_inputs["labels"].append(labels)
                model_inputs["task"].append(task_id)
    ...
    if training_args.do_train:
        ...
        if model_args.model_name_or_path != "resources/chatglmv2":
            model.enable_input_require_grads() # For ChatGLMv2
